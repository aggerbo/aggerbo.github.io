...
const I = {
  open: `<svg viewBox="0 0 460 230" xmlns="http://www.w3.org/2000/svg" font-family="-apple-system,Segoe UI,Roboto,sans-serif">
         <rect width="460" height="230" fill="#f2f8fd"/>`,
  close: `</svg>`,
  // label with a pointer line: I.lab(textX, textY, pointX, pointY, text, anchor)
  lab(tx, ty, px, py, text, anchor = "start") {
    return `<line x1="${tx}" y1="${ty + 4}" x2="${px}" y2="${py}" stroke="#d23c3c" stroke-width="1.2"/>
            <circle cx="${px}" cy="${py}" r="2.5" fill="#d23c3c"/>
            <text x="${tx}" y="${ty}" font-size="11.5" font-weight="600" fill="#0c2d4d" text-anchor="${anchor}">${text}</text>`;
  },
  title(t) {
    return `<text x="230" y="222" font-size="11" fill="#5b7387" text-anchor="middle">${t}</text>`;
  },
};

const NAVY = "#0c2d4d", SEA = "#0e6ba8", LIGHT = "#cfe5f4", METAL = "#9fb4c4", RUBBER = "#2b3a47";
...
const ILLUS = {
  ...
  impeller: () => I.open + `
    ...
    ${I.lab(295, 50, 205, 80, "rubber vanes — must be")}
    <text x="295" y="62" font-size="11.5" font-weight="600" fill="#0c2d4d">supple, none missing</text>
    ${I.lab(295, 120, 226, 118, "cover plate screws")}
    ${I.lab(295, 160, 172, 117, "brass hub (slides")}
    <text x="295" y="172" font-size="11.5" font-weight="600" fill="#0c2d4d">onto pump shaft)</text>
    ${I.title("Raw water pump with the round cover removed — the impeller sits inside")}` + I.close,

  strainer: () => I.open + `
    <rect x="140" y="38" width="90" height="18" rx="4" fill="${METAL}" stroke="${NAVY}" stroke-width="2"/>
    <rect x="150" y="56" width="70" height="110" rx="8" fill="#e8f4fc" stroke="${NAVY}" stroke-width="2.5"/>
    ...
    ${I.lab(300, 45, 226, 47, "lid — opens by hand", "start")}
    ${I.lab(300, 115, 214, 105, "basket: should look")}
    <text x="300" y="127" font-size="11.5" font-weight="600" fill="#0c2d4d">clear through the bowl</text>
    ${I.lab(40, 180, 70, 140, "from seacock —")}
    <text x="40" y="192" font-size="11.5" font-weight="600" fill="#0c2d4d">close before opening lid!</text>
    ${I.lab(300, 165, 285, 90, "to engine pump")}
    ${I.title("Sea strainer — seawater passes the basket before reaching the engine")}` + I.close,

  anodes: () => I.open + `
    <rect x="30" y="100" width="200" height="12" rx="6" fill="${METAL}" stroke="${NAVY}" stroke-width="2"/>
    <rect x="95" y="90" width="42" height="32" rx="6" fill="#b9c8d4" stroke="${NAVY}" stroke-width="2.5"/>
    ...
    <circle cx="313" cy="115" r="11" fill="#b9c8d4" stroke="${NAVY}" stroke-width="2.5"/>
    ${I.lab(60, 50, 112, 90, "shaft anode (collar)")}
    ${I.lab(150, 195, 245, 150, "propeller", "end")}
    ${I.lab(360, 40, 316, 62, "rudder", "end")}
    ${I.lab(365, 195, 324, 117, "rudder anode (disc)", "end")}
    ${I.title("Anodes corrode instead of your metal parts — replace at 50% gone")}` + I.close,
  ...
  antifoul: () => I.open + `
    ...
    <line x1="20" y1="118" x2="380" y2="118" stroke="${SEA}" stroke-width="2.5" stroke-dasharray="8 5"/>
    <rect x="40" y="80" width="320" height="15" fill="${LIGHT}" stroke="${NAVY}" stroke-width="2"/>
    ${I.lab(40, 40, 200, 87, "hull / boot stripe")}
    ${I.lab(310, 40, 350, 116, "waterline", "start")}
    ${I.lab(70, 200, 160, 160, "antifouling paint — everything below the waterline")}
    ${I.title("A clean painted bottom = full speed and lower diesel burn")}` + I.close,

  seacocks: () => I.open + `
    <rect x="30" y="160" width="340" height="14" fill="#cdb89a" stroke="${NAVY}" stroke-width="2"/>
    ...
    ${I.lab(300, 60, 255 ,68, "handle: ¼ turn,")}
    <text x="300" y="72" font-size="11.5" font-weight="600" fill="#0c2d4d">firm but smooth</text>
    ${I.lab(80, 80, 184, 75, "hose (double-clamped")}
    <text x="80" y="92" font-size="11.5" font-weight="600" fill="#0c2d4d">below waterline)</text>
    ${I.lab(70, 130, 178, 120, "bronze ball valve")}
    ${I.lab(300, 200, 228, 167, "through-hull fitting")}
    ${I.title("One of these guards every hole in the hull — know where they all are")}` + I.close,

  shaftseal: () => I.open + `
    <rect x="30" y="70" width="90" height="80" rx="10" fill="${LIGHT}" stroke="${NAVY}" stroke-width="2.5"/>
    <text x="75" y="115" font-size="12" font-weight="700" fill="${NAVY}" text-anchor="middle">gearbox</text>
    ...
    <path d="M250 60 L250 154 L380 175 L380 40 Z" fill="#cdb89a" stroke="${NAVY}" stroke-width="2.5" opacity=".9"/>
    <path d="M362 20 q 8 6 0 12 M376 20 q 8 6 0 12" stroke="${SEA}" stroke-width="2" fill="none"/>
    ${I.lab(110, 40, 180, 85, "rubber seal — squeeze to")}
    <text x="110" y="52" font-size="11.5" font-weight="600" fill="#0c2d4d">'burp' air out at launch</text>
    ${I.lab(60, 195, 210, 110, "propeller shaft")}
    ${I.lab(420, 195, 310, 130, "hull — sea outside", "end")}
    ${I.title("Where the shaft exits the hull. Should run dry — drips mean service time")}` + I.close,
  ...
  oilcheck: () => I.open + `
    ...
    <rect x="280" y="78" width="74" height="92" rx="10" fill="#eef7fd" stroke="${NAVY}" stroke-width="2.5"/>
    ...
    ${I.lab(40, 40, 122, 88, "dipstick (yellow loop):")}
    <text x="40" y="52" font-size="11.5" font-weight="600" fill="#0c2d4d">oil between the marks, never milky</text>
    ${I.lab(250, 40, 312, 60, "pressure cap — never")}
    <text x="250" y="52" font-size="11.5" font-weight="600" fill="#0c2d4d">open when hot!</text>
    ${I.lab(250, 205, 300, 145, "coolant level, checked cold")}
    ${I.title("The 60-second engine check before every trip")}` + I.close,

  fuelsep: () => I.open + `
    <rect x="150" y="30" width="100" height="22" rx="5" fill="${METAL}" stroke="${NAVY}" stroke-width="2"/>
    <rect x="158" y="52" width="84" height="90" rx="8" fill="#d8e2ea" stroke="${NAVY}" stroke-width="2.5"/>
    ...
    ${I.lab(60, 80, 156, 90, "filter element", "start")}
    ${I.lab(300, 130, 236, 152, "clear bowl: diesel...")}
    ${I.lab(300, 168, 236, 178, "...water sinks below")}
    ${I.lab(300, 205, 210, 198, "drain screw — let water out")}
    ${I.lab(60, 200, 100, 47, "from tank")}
    ${I.title("Water separator — check the bowl: any water layer gets drained out")}` + I.close,
  ...
  belts: () => I.open + `
    ...
    <path d="M180 62 l6 14 M196 58 l5 14 M212 56 l4 14" stroke="#d23c3c" stroke-width="2.5"/>
    ${I.lab(50, 50, 90, 110, "crank pulley")}
    ${I.lab(330, 40, 268, 60, "alternator", "start")}
    ${I.lab(340, 205, 322, 182, "pump", "end")}
    ${I.lab(120, 30, 196, 64, "look for cracks &amp;")}
    <text x="120" y="42" font-size="11.5" font-weight="600" fill="#0c2d4d">black dust below</text>
    ${I.title("Press midway between pulleys — about 1 cm of give is right")}` + I.close,
  ...
  batteries: () => I.open + `
    ...
    <text x="125" y="130" font-size="13" font-weight="700" fill="${NAVY}" text-anchor="middle">START</text>
    ...
    <text x="285" y="130" font-size="13" font-weight="700" fill="${NAVY}" text-anchor="middle">HOUSE</text>
    <rect x="92" y="142" width="66" height="20" rx="5" fill="#fff" stroke="${NAVY}" stroke-width="1.5"/>
    <text x="125" y="157" font-size="12" font-weight="700" fill="#1d9f6e" text-anchor="middle">12.6 V</text>
    ${I.lab(40, 40, 88, 64, "+ terminal: tight,")}
    <text x="40" y="52" font-size="11.5" font-weight="600" fill="#0c2d4d">no green fuzz</text>
    ${I.lab(290, 40, 285, 78, "house bank: fridge,", "start")}
    <text x="290" y="52" font-size="11.5" font-weight="600" fill="#0c2d4d">lights, instruments</text>
    ${I.lab(60, 205, 120, 165, "rested 12.6 V+ = healthy · below 12.2 V = charge it")}
    ${I.title("Separate banks so the fridge can never strand you")}` + I.close,
  ...
  bilge: () => I.open + `
    ...
    ${I.lab(70, 80, 188 ,135, "bilge pump")}
    ${I.lab(310, 110, 252, 142, "float switch — lift it", "start")}
    <text x="310" y="122" font-size="11.5" font-weight="600" fill="#0c2d4d">by hand: pump must run</text>
    ${I.lab(300, 30, 320, 54, "out through hull side", "start")}
    ${I.lab(70, 200, 160, 162, "lowest point of the hull — should be DRY")}
    ${I.title("New water in the bilge always has a story — find it")}` + I.close,
  ...
  trimtabs: () => I.open + `
    ...
    <circle cx="160" cy="143" r="5" fill="#b9c8d4" stroke="${NAVY}" stroke-width="1.5"/>
    <circle cx="240" cy="143" r="5" fill="#b9c8d4" stroke="${NAVY}" stroke-width="1.5"/>
    <line x1="20" y1="160" x2="380" y2="160" stroke="${SEA}" stroke-width="2.5" stroke-dasharray="8 5"/>
    ${I.lab(40, 80, 128, 125, "hydraulic ram")}
    ${I.lab(330, 80, 270, 136, "steel plate, hinged", "start")}
    ${I.lab(60, 200, 158, 146, "small anode on each tab")}
    ${I.title("Transom seen from behind — tabs press the stern up to level the ride")}` + I.close,

  thruster: () => I.open + `
    <path d="M380 40 Q 180 48 100 80 Q 50 100 44 150 L 380 150 Z" fill="${LIGHT}" stroke="${NAVY}" stroke-width="2.5"/>
    <line x1="20" y1="95" x2="395" y2="95" stroke="${SEA}" stroke-width="2.5" stroke-dasharray="8 5"/>
    ...
    <circle cx="108" cy="123" r="4" fill="#b9c8d4" stroke="${NAVY}" stroke-width="1.5"/>
    ${I.lab(40, 50, 90, 107, "tunnel through the bow")}
    ${I.lab(240, 195, 112, 125, "prop + small anode — keep free of weed &amp; line")}
    ${I.lab(310, 70, 330, 92, "waterline", "start")}
    ${I.title("Bow seen from the side — the thruster pushes the bow sideways")}` + I.close,
  ...
  heater: () => I.open + `
    ...
    ${I.lab(60, 50, 150, 92, "burner — diesel from")}
    <text x="60" y="62" font-size="11.5" font-weight="600" fill="#0c2d4d">the main tank</text>
    ${I.lab(300, 60, 290, 90, "warm air ducts to cabins", "start")}
    ${I.lab(40, 205, 56, 170, "exhaust through hull side")}
    ${I.lab(300, 205, 235, 180, "fuel pump (ticks when running)")}
    ${I.title("Run it hard 20 min a month so it never cokes up")}` + I.close,

  rudder: () => I.open + `
    <rect x="30" y="60" width="340" height="14" fill="#cdb89a" stroke="${NAVY}" stroke-width="2"/>
    ...
    ${I.lab(60, 40, 184, 40, "rudder bearing &amp; tiller arm")}
    <text x="60" y="52" font-size="11.5" font-weight="600" fill="#0c2d4d">(inside the boat)</text>
    ${I.lab(300, 110, 212, 100, "rudder stock — grab the", "start")}
    <text x="300" y="122" font-size="11.5" font-weight="600" fill="#0c2d4d">blade &amp; feel for play</text>
    ${I.lab(80, 195, 190, 170, "rudder blade")}
    ${I.title("At haul-out: push the rudder by hand — clunky play = worn bearing")}` + I.close,
};
